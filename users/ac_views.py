...
from .models import User, CmeTag, State, Hospital, AllowedUrl, EligibleSite
...
class CmeTagAutocomplete(autocomplete.Select2QuerySetView):
    def get_queryset(self):
        ...
        if self.q:
            qs = qs.filter(name__istartswith=self.q, srcme_only=False)
        return qs
...
class AllowedUrlAutocomplete(autocomplete.Select2QuerySetView):
    def get_queryset(self):
        ...
        if self.q:
            qs = qs.filter(url__startswith=self.q)
        return qs
...
class EligibleSiteAutocomplete(autocomplete.Select2QuerySetView):
    def get_queryset(self):
        ...
        if self.q:
            qs = qs.filter(domain_title__istartswith=self.q)
        return qs
